class PokemonSummary_Scene
  def drawPageThree
    ...
    statbase = {}
    GameData::Stat.each_main { |s| statbase[s.id] = base }
    if !@pokemon.shadowPokemon? || @pokemon.heartStage > 3
      @pokemon.nature_for_stats.stat_changes.each do |change|
        statbase[change[0]] = Color.new(255,189,152) if change[1] > 0
        statbase[change[0]] = Color.new(114,145,176) if change[1] < 0
      end
    end
    statshadows = {}
    ...
    if !@pokemon.shadowPokemon? || @pokemon.heartStage > 3
      @pokemon.nature_for_stats.stat_changes.each do |change|
        statshadows[change[0]] = Color.new(200,70,21) if change[1] > 0
        statshadows[change[0]] = Color.new(40,71,105) if change[1] < 0
      end
    end
    # Write various bits of text
    textpos = [
       [_INTL("HP"),243,70,2,statbase[:HP],statshadows[:HP]],
       ...
       [_INTL("Attack"),225,114,0,statbase[:ATTACK],statshadows[:ATTACK]],
       ...
       [_INTL("Defense"),225,146,0,statbase[:DEFENSE],statshadows[:DEFENSE]],
       ...
       [_INTL("Sp. Atk"),225,178,0,statbase[:SPECIAL_ATTACK],statshadows[:SPECIAL_ATTACK]],
       ...
       [_INTL("Sp. Def"),225,210,0,statbase[:SPECIAL_DEFENSE],statshadows[:SPECIAL_DEFENSE]],
       ...
       [_INTL("Speed"),225,242,0,statbase[:SPEED],statshadows[:SPEED]],
    ...
    if ability
      textpos.push([ability.name,354,278,0,Color.new(64,64,64),Color.new(176,176,176)])
