...
if Settings::EBDX_COMPAT  
  EliteBattle.configProcess(:UI) do
    ...
    for section in metrics.keys
      if ["PLAYERDATABOX", "ENEMYDATABOX", "BOSSDATABOX", "RAIDDATABOX"].include?(section.upcase)
        options = {}
      ...
      # configure fight menu
        ...
        options = {}
        # primary section
        for set in ['BarGraphic', 'SelectorGraphic', 'ButtonGraphic', 'MegaButtonGraphic', 'ZMoveButtonGraphic', 'UltraButtonGraphic', 'DynaButtonGraphic', 'TypeGraphic', 'CategoryGraphic', 'ShowTypeAdvantage']
          options[set.upcase.to_sym] = metrics[section]["__pk__"][set][0] if metrics[section]["__pk__"] && metrics[section]["__pk__"][set]
        end
        # register options
        EliteBattle.add_data(section.upcase.to_sym, :METRICS, options)
