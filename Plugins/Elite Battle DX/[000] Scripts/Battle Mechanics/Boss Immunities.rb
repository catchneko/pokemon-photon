...
#  Endeavor immunity
#===============================================================================
class PokeBattle_Move_06E
  alias endeavor_ebdx pbFailsAgainstTarget? unless self.method_defined?(:endeavor_ebdx)
  def pbFailsAgainstTarget?(*args)
    rule = EliteBattle.get_data(:BOSSBATTLES, :Metrics, :IMMUNITIES)
    rule = rule.nil? ? false : rule.include?(:ENDEAVOR)
    if args[1] && args[1].immunity && rule
      @battle.pbDisplay(_INTL("{1} is unaffected!", args[1].pbThis))
      return true
    end
    return endeavor_ebdx(*args)
  end
end
#===============================================================================
#  Super Fang immunity
#===============================================================================
class PokeBattle_Move_06C
  alias superfang_ebdx pbFailsAgainstTarget? unless self.method_defined?(:superfang_ebdx)
  def pbFailsAgainstTarget?(*args)
    rule = EliteBattle.get_data(:BOSSBATTLES, :Metrics, :IMMUNITIES)
    rule = rule.nil? ? false : rule.include?(:SUPERFANG)
    if args[1] && args[1].immunity && rule
      @battle.pbDisplay(_INTL("{1} is unaffected!", args[1].pbThis))
      return true
    end
    return superfang_ebdx(*args)
  end
end
#===============================================================================
#  Pain Split immunity
#===============================================================================
class PokeBattle_Move_05A
  alias painsplit_ebdx pbFailsAgainstTarget? unless self.method_defined?(:painsplit_ebdx)
  def pbFailsAgainstTarget?(*args)
    rule = EliteBattle.get_data(:BOSSBATTLES, :Metrics, :IMMUNITIES)
    rule = rule.nil? ? false : rule.include?(:PAINSPLIT)
    if args[1] && args[1].immunity && rule
      @battle.pbDisplay(_INTL("{1} is unaffected!", args[1].pbThis))
      return true
    end
    return painsplit_ebdx(*args)
  end
end
